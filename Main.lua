...
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/SNscript-up/Library-HYPER-MM/refs/heads/main/main.lua"))()
...
AutoFarm:AddSwitch("Auto Rebirth", function(state)
    getgenv()._AutoRepFarmEnabled = state

    while getgenv()._AutoRepFarmEnabled do
        for _,v in pairs(game:GetDescendants()) do
            
            if v:IsA("RemoteEvent") and string.find(v.Name:lower(),"rebirth") then
                v:FireServer()
            end
...
AddSwitch("Lock Position", function(Value)
    if Value then
...
Credits:AddLabel("H  BY SN")
Credits:AddLabel("Script Hecho por: SN")
Credits:AddLabel("HYPER H")
